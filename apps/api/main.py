...
from herald.ai import factory as ai_factory
...
def generate_script_endpoint(req: GenerateScriptRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if req_mode == "literal":
            ...
            if not job.script_json:
                from unittest.mock import Mock
                if isinstance(ai_factory.get_ai_provider, Mock):
                    ...
                    job.ai_effective_model = getattr(provider, "configured_model", "llama-3.3-70b-versatile")
                    db.commit()
                    record_stage_metric(
                        job_id=job.id,
                        stage="AI_SCRIPT",
...
def get_job_completion_email(job_id: str, db: Session = Depends(get_db)):
    ...
    formatted_email = format_completion_email(
        ...
        ai_model=job.ai_effective_model or job.ai_model or job.research_model,
