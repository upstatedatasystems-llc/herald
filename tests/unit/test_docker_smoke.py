...
def test_dockerfiles_structure():
    """Verify Dockerfile.worker copies correct directory structure, does not reference packages/, and Dockerfile.api is retired."""
    root_dir = Path(__file__).parent.parent.parent
    docker_dir = root_dir / "docker"
    compose_file = root_dir / "compose.yaml"

    assert docker_dir.exists()
    assert not (docker_dir / "Dockerfile.api").exists()
