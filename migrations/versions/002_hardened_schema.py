...
def upgrade() -> None:
    ...
    op.add_column(
        "podcast_jobs",
        sa.Column("synthesis_attempt_count", sa.Integer(), server_default="0", nullable=False),
    )
    op.add_column(
        "podcast_jobs",
    ...
    op.add_column(
        "podcast_jobs", sa.Column("next_retry_at", sa.DateTime(timezone=True), nullable=True)
    )

    op.add_column(
        "podcast_jobs", sa.Column("claimed_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column("podcast_jobs", sa.Column("claim_owner", sa.String(length=100), nullable=True))
    op.add_column(
        "podcast_jobs", sa.Column("last_heartbeat_at", sa.DateTime(timezone=True), nullable=True)
    )

    op.add_column(
        "podcast_jobs", sa.Column("drive_uploaded_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "podcast_jobs", sa.Column("delivered_at", sa.DateTime(timezone=True), nullable=True)
    )
