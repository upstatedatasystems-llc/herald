...
def upgrade() -> None:
    with op.batch_alter_table('podcast_jobs') as batch_op:
        batch_op.add_column(sa.Column('tts_chunk_chars', sa.Integer(), nullable=True, server_default='500'))
        batch_op.add_column(sa.Column('verify_final_script', sa.Boolean(), nullable=True, server_default='false'))
        batch_op.add_column(sa.Column('verify_audit_json', sa.JSON(), nullable=True))
        batch_op.add_column(sa.Column('verify_repair_count', sa.Integer(), nullable=False, server_default='0'))
        batch_op.add_column(sa.Column('tts_resource_metrics_json', sa.JSON(), nullable=True))
        batch_op.add_column(sa.Column('details_finalized_at', sa.DateTime(timezone=True), nullable=True))
