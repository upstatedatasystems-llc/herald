...
def upgrade() -> None:
    ...
    with op.batch_alter_table('podcast_jobs') as batch_op:
        ...
        batch_op.create_index('uq_podcast_jobs_telegram', ['transport', 'telegram_chat_id', 'telegram_message_id'], unique=True)
    ...
    op.create_index('idx_telegram_update_failures_dead_letter', 'telegram_update_failures', ['is_dead_lettered'])
...
def downgrade() -> None:
    ...
    null_rows = conn.execute(
        sa.text("SELECT COUNT(*) FROM podcast_jobs WHERE gmail_message_id IS NULL OR sender_email IS NULL")
    ).scalar()
