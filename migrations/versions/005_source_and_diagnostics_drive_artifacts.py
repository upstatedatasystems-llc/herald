...
def upgrade() -> None:
    with op.batch_alter_table('podcast_jobs') as batch_op:
        ...
        batch_op.add_column(sa.Column('source_drive_web_link', sa.String(length=512), nullable=True))
        batch_op.add_column(sa.Column('diagnostics_drive_file_id', sa.String(length=255), nullable=True))
        batch_op.add_column(sa.Column('diagnostics_drive_web_link', sa.String(length=512), nullable=True))
        ...
        batch_op.create_index('idx_podcast_jobs_source_drive_file_id', ['source_drive_file_id'], unique=True)
        batch_op.create_index('idx_podcast_jobs_diagnostics_drive_file_id', ['diagnostics_drive_file_id'], unique=True)
