...
def upgrade() -> None:
    with op.batch_alter_table("podcast_jobs") as batch_op:
        ...
        batch_op.add_column(
            sa.Column("research_repair_count", sa.Integer(), nullable=False, server_default="0")
        )
        batch_op.add_column(sa.Column("script_drive_file_id", sa.String(length=255), nullable=True))
        batch_op.add_column(
            sa.Column("script_drive_web_link", sa.String(length=512), nullable=True)
        )
        batch_op.add_column(
            sa.Column("research_drive_file_id", sa.String(length=255), nullable=True)
        )
        batch_op.add_column(
            sa.Column("research_drive_web_link", sa.String(length=512), nullable=True)
        )
        batch_op.add_column(
            sa.Column("research_notes_drive_file_id", sa.String(length=255), nullable=True)
        )
        batch_op.add_column(
            sa.Column("research_notes_drive_web_link", sa.String(length=512), nullable=True)
        )

        batch_op.create_index(
            "idx_podcast_jobs_script_drive_file_id", ["script_drive_file_id"], unique=True
        )
        batch_op.create_index(
            "idx_podcast_jobs_research_drive_file_id", ["research_drive_file_id"], unique=True
        )
        batch_op.create_index(
            "idx_podcast_jobs_research_notes_drive_file_id",
            ["research_notes_drive_file_id"],
            unique=True,
        )
