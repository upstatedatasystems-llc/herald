...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "job_id",
            sa.String(length=36),
            sa.ForeignKey("podcast_jobs.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "idx_job_processing_metrics_job_stage", "job_processing_metrics", ["job_id", "stage"]
    )
    op.create_index(
    ...
    op.create_index(
        "idx_job_processing_metrics_job_seq", "job_processing_metrics", ["job_id", "sequence_index"]
    )
    ...
    with op.batch_alter_table("podcast_jobs") as batch_op:
        ...
        batch_op.add_column(
            sa.Column("details_drive_file_id", sa.String(length=255), nullable=True)
        )
        batch_op.add_column(
            sa.Column("details_drive_web_link", sa.String(length=512), nullable=True)
        )
        batch_op.create_index(
            "idx_podcast_jobs_details_drive_file_id", ["details_drive_file_id"], unique=True
        )
